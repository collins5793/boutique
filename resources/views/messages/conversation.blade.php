...
<div class="container py-4 chat-wrapper">
    <h3 class="mb-3">💬 Conversation avec {{ $client->name }}</h3>
    ...
    <form id="chatForm" action="{{ route('messages.store', $client->id) }}" method="POST" enctype="multipart/form-data" class="composer">
        @csrf
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function fetchNewMessages() {
        fetch("{{ route('messages.fetch', $client->id) }}?after=" + lastMessageId, {
    ...
    function markMessagesAsRead() {
        fetch("{{ route('messages.read', $client->id) }}", {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'X-Requested-With': 'XMLHttpRequest'
